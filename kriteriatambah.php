...
<body>
  ...
  if (isset($_POST['simpan'])) {
    ...
    $cek = mysqli_query($koneksi, "SELECT nama_kriteria FROM tab_kriteria WHERE nama_kriteria ='$kriteria'");
    if ($cek->num_rows > 0) {
      header("location:kriteria.php?pesan=nama_kriteria_sama");
    } else {
      $masuk = "INSERT INTO tab_kriteria VALUES ('$id_krit','$kriteria','$bobot')";
      $buat  = $koneksi->query($masuk);
      if ($buat) {
        header("location:kriteria.php?pesan=save");
      }
    }
  }
